...
collectionList = db['GitHub_Users_DavidWoods']  # Replace 'mycollection' with your collection name
...
while True:
    ...
    data = requestToGit(
        ...
        {
            'q': 'type:User in:name "David" AND "Woods" created:>'+dateJoinedFrom,
            'sort': 'joined', 'order': 'asc', 'page': page, 'per_page': 30
        })
    print(f"total Count: {data['total_count']}, currentPage : {page}, catched_count : {origin_count+(page-1)*30}")

    # Store each user's login name for detailed profile fetch
    user_logins = [user['login'] for user in data.get('items', [])]
